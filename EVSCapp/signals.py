...
#track the vehicles and send new notification if traffic police did not receive the notification
@receiver(post_save,sender = VehicleTracker)
def send_notification_to_new_traffic(sender, instance, created, **kwargs):
    if created:
        ...
        if record and record.status == False:
            ...
            if len(traffic_police_location_list) > 0:
                ...
                record_latitude = instance.records.latitude
                record_longitude = instance.records.longitude
                radius = 6371
                for traffic_police_pair in traffic_police_location_list:
                    traffic_latitude = traffic_police_pair["latitude"]
                    traffic_longitude = traffic_police_pair["longitude"]
                ...
                if Notification.objects.filter(recipient=nearby_traffic_police,records=instance.records).exists():
                    print("Traffic Police already exists")
                else:
                    notification = Notification.objects.create(recipient = nearby_traffic_police, records = instance.records, content = "message_body")
                    notification.save()
